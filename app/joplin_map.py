...
def create_joplin_map():
    """Create an interactive map of Joplin tornado damage"""
    # Path to Joplin data
    base_path = r'C:\Users\yanit\OneDrive\Bureau\Data Insights And Visualization\tier3\labels'
    labels_generator = Path(base_path).rglob(pattern='joplin-tornado_*post_*.json')
    
    # Get all Joplin labels
    joplin_labels = [str(label) for label in labels_generator]
    
    if not joplin_labels:
        print("No Joplin data found. Please check the data path.")
        return None
    
    # Generate metadata dataframe
    df = generate_metadata_df(joplin_labels)
...
if __name__ == "__main__":
    # Create the map
    joplin_map = create_joplin_map()
    
    if joplin_map:
        # Save the map to an HTML file
        joplin_map.save('joplin_test_tornado_map.html')
        print("Map saved as 'joplin_test_tornado_map.html'")
    else:

...
def create_response_time_map():
    # Load data
    hospitals_gdf = gpd.read_file(r'C:\Users\yanit\OneDrive\Bureau\Data Insights And Visualization\tsunami\sunda_hospital.geojson')
    roads_gdf = gpd.read_file(r'C:\Users\yanit\OneDrive\Bureau\Data Insights And Visualization\tsunami\sunda_roads.geojson')
    ...
    # Plot ocean first (light blue color)
    ocean_color = '#E6F3FF'  # Light blue for ocean
    ax.fill_between([105.79357115956361, 105.88173137250544], 
                   -6.4306412229426115, -6.211238407940371,
    ...
    poi_df = pd.read_csv(r'C:\Users\yanit\OneDrive\Bureau\Data Insights And Visualization\tsunami\poi_sunda.csv')
    ax.scatter(poi_df['centroid_x'], poi_df['centroid_y'], 
               c='red', s=50, alpha=0.7, label='POI Centroids')
    
    # Update axis limits to match the full extent of the Sunda area
    ax.set_xlim(105.79357115956361, 105.88173137250544)
    ax.set_ylim(-6.4306412229426115, -6.211238407940371)

    plt.title('Emergency Response Time Map - Sunda Area')
